...
roleController = function($scope, dataStorage, resourceService, objectProperties){
    $scope.showEditForm = false;
    $scope.showListForm = true;
...
editRoleController = function($scope, dataStorage, resourceService, objectProperties) {
    ...
    function updateForm(){
        var entityEditForm = new EntityEditForm();
        entityEditForm.editFormName = "New role:";
        entityEditForm.formProperties = objectProperties.getRoleObjectProperties().formProperties;
        entityEditForm.currentEntity = dataStorage.getCurrentRole();
    ...
    function createEntity(template){
        var entityList = dataStorage.getRoleList();
        entityList.addEntityByTemplate(resourceService, template, function(){
            dataStorage.setRoleList(entityList);
            closeEditForm();
        });
    }
...
roleListController = function($scope, dataStorage, resourceService, objectProperties){
    ...
    function updateForm(){
        ...
        entityListForm.editFormName = "Role list:";
        entityListForm.formProperties = objectProperties.getRoleObjectProperties().listProperties;
    ...
    function addNewEntity(){
        openEditForm(new Role(dataStorage))
    }

    function editEntity(id){
        var entity = dataStorage.getRoleList().findEntityById(id);
        if(entity != undefined){
            var editEntity = new Role(dataStorage);
            appUtils.fillValuesProperty(entity, editEntity);
            openEditForm(editEntity);
        }
    }
    ...
    function openEditForm(currentEntity){
        dataStorage.setCurrentRole(currentEntity);
        closeListForm();
    }
